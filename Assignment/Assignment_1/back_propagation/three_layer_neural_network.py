...
class NeuralNetwork(object):
    ...
    def backprop(self, X, y):
        ...
        delta3 = self.probs
        delta3[range(num_examples), y] -= 1  # Subtract 1 from the true class probabilities
    
        # Step 2: Compute gradients for W2 and b2
        dW2 = np.dot(self.a1.T, delta3)
        db2 = np.sum(delta3, axis=0, keepdims=True)
    
        # Step 3: Compute the delta for the hidden layer
        delta2 = np.dot(delta3, self.W2.T) * self.diff_actFun(self.z1, self.actFun_type)
    
        # Step 4: Compute gradients for W1 and b1
...
def main():
    ...
    #Pick 1 one to activate
    # model = NeuralNetwork(nn_input_dim=2, nn_hidden_dim=10 , nn_output_dim=2, actFun_type='tanh')
    
    # model = NeuralNetwork(nn_input_dim=2, nn_hidden_dim=10, nn_output_dim=2, actFun_type='sigmoid')
